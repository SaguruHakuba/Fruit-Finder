...
%orange
hsvmask = zeros(size(hsvimg(:,:,1)));
hsvmask(find((h <= 0.115 & h >= 0.05) & (s >= 0.4)& (v >= 0.5))) = 1;
rgbmask(:,:,2) = hsvmask;

%banana
hsvmask = zeros(size(hsvimg(:,:,1)));
hsvmask(find((h <= 0.25 & h >= 0.13) & (s >= 0.5)& (v >= 0.3))) = 1;
rgbmask(:,:,3) = hsvmask;

% find the connected regions:
BW = bwconncomp(rgbmask(:,:,1));
...
figure(7);
imshowpair(rgbmask,Test,'montage');

imwrite(rgbmask,'test.tiff');
